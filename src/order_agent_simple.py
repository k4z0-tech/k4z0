"""
Order Analysis Agent (Simplified version for Windows compatibility)
...
class OrderAgent:
    ...
    def load_data(self):
        ...
        try:
            ...
            print(f"[INFO] Columns: {list(self.orders_df.columns)}")
    ...
    def save_feedback_history(self):
        ...
        print(f"[SAVED] Feedback history saved to {self.feedback_file}")
    ...
    def analyze_delivery_tat(self, tat_threshold_days: int = 3) -> List[Dict[str, Any]]:
        ...
        if tat_column is None:
            print("[ERROR] Could not find 'Delivery TAT' column in the data")
    ...
    def provide_guidance(self, order_id: str) -> str:
        ...
        """
        if not self.learned_actions:
            return "[INFO] No previous feedback available yet. Please provide feedback to help me learn."

        similar_actions = self.get_similar_actions(order_id)
        if not similar_actions:
            return "[INFO] No similar actions found for this order type."
    ...
    def run_analysis(self):
        ...
        for order in self.delayed_orders:
            ...
            print(f"\n[OK] Feedback recorded for order {order_id}")
        ...
        print(f"[STATS] Collected feedback for all delayed orders")
